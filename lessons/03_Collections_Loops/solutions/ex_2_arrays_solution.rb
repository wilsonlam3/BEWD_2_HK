# ====================================================================
# EVERYTHING BELOW THIS LINE IS SETUP CODE, YOU DON'T NEED TO CHANGE IT
# ====================================================================
...
assert_equal(5, categories.length) # get its length (.size, .count are also valid)

# Remove the newest category from the array, print it to the screen "Last Category: Category here"

last_category = categories.delete_at(-1)
assert_equal("Last Category: Weather", "Last Category: #{last_category}")
assert_equal(4, categories.size) # get its length

# Write a conditional that clears the array if there are more than 5 categories and adds "Misc" to it.

if categories.count > 5 # get its length
  categories.clear # empty the array
  categories << "Misc"
end

assert_equal(4, categories.length) # get its length

categories << "Medicine"
categories << "Physics"

if categories.length > 5 # get its length
  categories.clear # empty the array
  categories << "Misc"
end

assert_equal(1, categories.length) # get its length
